    ...
    def post(self, request):
        ...
        if not User.objects.filter(username=username).exists():
            if not User.objects.filter(email=email).exists():
                ...
                email_body = "Hiii " + user.username + ", Please use this linnk to verify your account\n" + activate_url
...
def account_view(request):
    return render(request, 'authentication/account.html', {'user': request.user})